...
```Sh
# start the job
az containerapp job start --name ca-neshop-server-migrations --resource-group rg-dev-neshop --output json
```

```Sh
# list all executions of the job
az containerapp job execution list --name ca-neshop-server-migrations --resource-group rg-dev-neshop
```
